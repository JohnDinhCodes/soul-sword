...
class HomeTown extends Phaser.Scene {
    ...
    create() {
        ...
        // Layers
        this.mapLayer.push(map.createStaticLayer("Ground", tilesetArr, 0, 0));
        this.mapLayer.push(
            map.createStaticLayer("Obstacles", tilesetArr, 0, 0)
        );
        this.mapLayer.push(
            map.createStaticLayer("House Below", tilesetArr, 0, 0)
        );
        ...
        this.mapLayer.push(
            map.createStaticLayer("House Above", tilesetArr, 0, 0)
        );
        ...
        if (this.isMobile) {
            ...
            this.input.keyboard.on("keydown_X", () => {
                if (this.dialogueActionButton) {
                    this.actionButton(textBlip);
                }
                this.dialogueActionButton = false;
            });
            this.input.keyboard.on("keyup_X", () => {
                this.dialogueActionButton = true;
            });
    ...
    update(time, delta) {
        ...
        this.characterMovement.npcMovement(this.NPCs[this.NPCs.lumberjack], [
            { direction: "right", value: 48 },
            { direction: "left", value: 48 }
        ]);
